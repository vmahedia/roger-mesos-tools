...
class RogerPush(object):
    ...
    def parse_args(self):
      self.parser = argparse.ArgumentParser(prog='roger push', description=describe())
      ...
      self.parser.add_argument('--skip-push', '-s', help="skips push. Only generates components for review. Defaults to false.", action="store_true")
      self.parser.add_argument('--force-push', '-f', help="force push. Not Recommended. Forces push even if validation checks failed. Defaults to false.", action="store_true")
      ...
      for key in output_dict:
        ...
        if type(output_dict[key]) is list:
          ...
          for list_elem in output_dict[key]:
            temp_list.append(self.replaceSecrets(list_elem, secrets_dict))
    ...
    def main(self, settings, appConfig, frameworkObject, hooksObj, args):
      ...
      for container in data_containers:
        ...
        if container_name not in failed_container_list:
          ...
          if 'SECRET' in output:
            output = self.mergeSecrets(output, self.loadSecretsJson(secrets_dir, containerConfig, args, environment))

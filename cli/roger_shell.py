...
class RogerShell(object):
    ...
    def parse_args(self):
      self.parser = argparse.ArgumentParser(prog='roger shell', description=describe())
    ...
    then will log into the first one")
        print("Executing bash in docker container - {0} on host - {1} for mesosTaskId - {2}".format(containerId, hostname, mesosTaskId));
        try:
